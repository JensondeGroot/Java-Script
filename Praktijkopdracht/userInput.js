...
// check rollValue op welke getalen en geef dat aan de kind functie
...
function setPlaceData(player, i, totals) {
    if (!gameData.p1IsPlaced[player - 1][i] && !gameData.p2IsPlaced[player - 1][i]) {
        ...
        if (i >= 10 && i <= 16) {
            //Of a kinds
            if (i == 10) scoreBlock(player, 10).innerText = gameData.calP2[player - 1][0] ? addToP2(valAllDice()) : '❌';
            if (i == 11) scoreBlock(player, 11).innerText = gameData.calP2[player - 1][1] ? addToP2(valAllDice()) : '❌';
            ...
            //chance
            if (i == 16) scoreBlock(player, 16).innerText = addToP2(valAllDice());

            gameData.p2IsPlaced[player - 1][i] = true;

            //things to add to totalP2
